...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Les résultats de l'analyse d'expression différentielle a mis en valeur 2181 gènes differentiellement exprimés au risque alpha = 0.05 (figure 3), 1242 gènes sont sur-exprimés et 939 sous-exprimés par rapport à la condition de réference "control". 
...
tb.DE
```
\sffamily\small{Figure 3 : Table des résultats de l'analyse d'expression différentielle au risque alpha = 0.05}
\normalsize
\setromanfont{Times New Roman}
\setstretch{1.5}

On remarque que les gènes les plus significativement différentiellement exprimés sont SPACL1, PER1, ARHGEF2,et MAOA, la plus part des gènes les plus différentiellement exprimés sont surexprimés par rapport à la réference, on note d'ailleurs que les deux gènes les plus sur exprimés sont ALOX15B et AC007325.2 (figure 4)
...
\sffamily\small{Figure 4 : Volcano plot de l'analyse d'expression différentielle.
...
```
\sffamily\small{Figure 5 : Analyse en composante principale (gauche) et matrice de distance (droite) sur les 8 échantillons avec une transformation selon la méthode VST.
...
```
\sffamily\small{Figure 6 : Heatmap de l'expression des 50 gènes ayant la meilleur p.value. Le gradient utilisé est un gradient rouge-vert. Plus l'expression est importante plus la couleur vire au rouge et inversement plus la couleur vire au vert plus l'expression est faible.}
\normalsize
