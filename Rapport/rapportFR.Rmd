...
```{r message=FALSE, warnings=FALSE, eval=TRUE, tidy=FALSE , echo =FALSE}
...
\scriptsize
```{r message=FALSE, warnings=FALSE, eval=TRUE, tidy=FALSE}
### Create dds object ---
...
```
\normalsize
\small{Figure ? : Table des résultats de l'analyse d'expression différentielle
}
\normalsize
...
```
\normalsize
\small{Figure ? : Volcano plot de l'analyse d'expression différentielle.
En bleu on retrouve les gènes dont l'expression diffère de la condition de réference "control" au risque alpha = 0.o5 et en rouge les gènes dont l'expression ne diffère pas au risque alpha = 0.05. Les points à droite de la ligne de démarcation correspondent aux gènes surexprimés par rapport à la réference et les points à gauche correspondent aux gènes sous exprimés par rapport à la référence. }
\normalsize
```{r message=FALSE, warnings=FALSE, eval=TRUE, tidy=FALSE, echo=FALSE }
#PCA
...
```
\normalsize
\small{Figure ? : Analyse en composante principale et matrice de distance sur les 8 échantillons avec une transformation selon la méthode vsdata.
...
```
\normalsize
\small{Figure ? : Heatmap de l'expression des 50 gènes ayant la plus la meilleur p.value. Le gradient utilisé est un gradient rouge-vert, plus l'expression est importante plus cela vire au rouge et inversement plus cela vire au vert plus l'expression est faible.  
}
\normalsize
