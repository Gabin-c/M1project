...
### Depth plot ----
### depth.plot return a barplot of depth sample with ggplot library  
### depth.plot need two argument
###   - dds.count which is a count table of a RNAseq experience which have in column : sample, and in row :  gene
###   - break.width which is width of the bar
...
### Count distribution plot ----
### count.distribution return an histogram of count values distribution in the log(count+1) format for one sample
### count.distribution need five arguments
###   - dds.count which is a count table of an RNAseq experience which have in column : sample, and in row :  gene
###   - break.width which is width of histogram bar
###   - sample for which we display the count distribution
###   - x.min which is the min of x axis 
###   - x.max which is the max of x axis
...
### Number of differemtial express gene ----
...
count.plot <- function(dds.count,gene){
  count.tb <- as.data.frame(t(dds.count))
  count.tb[,"sample"] <- row.names(count.tb)
  return(
    ggplot(count.tb, aes(x=count.tb[,"sample"], y=count.tb[,gene])) + 
      geom_point(size=4,aes(colour=factor(sample)) + 
      geom_segment(aes(x=count.tb[,"sample"], xend=count.tb[,"sample"], y=0, yend=count.tb[,gene]),linetype="dotdash")+ 
